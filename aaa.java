	...
	public static void main(String[] args){
        ...
        for(int i = 1 ; i<=totalMember ; i++){//i 會等於totalmember，代表知道有幾個成員
            for (int j=1;j<=totalMember;j++){//j要計算除了本身以外的運算 ，j代表儲存了位址並非成員的編號
				...
				if(j!=i){
					mon = (num[i] - num[j]);
					// System.out.println("這邊顯示mon :" + mon);
					upon = upon * (-num[j]);
	                // System.out.println("這邊顯示upon" + upon);
					
					montotal = montotal * mon;
					// System.out.println("這邊顯示montotal" + montotal);
			...
			if(montotal<0){
				...
				// System.out.println("加法反元素的mon:" + mon);
					}
			}
			...
			qq[i] = inv[i].solve(montotal,modnumber);
			if(qq[i]==0){
				qq[i] +=1;
			}
 			xxx = upon * qq[i];
